...
function doRectanglesOverlap(rect1, rect2) {
  const oneObjHorizontal = [rect1.left, rect1.left + rect1.width];
  const twoObjHorizontal = [rect2.left, rect2.left + rect2.width];
  const oneObjVertical = [rect1.top, rect1.top + rect1.height];
  const twoObjVertical = [rect2.top, rect2.top + rect2.height];
  if (oneObjHorizontal[1] < twoObjHorizontal[0] || oneObjHorizontal[0] > twoObjHorizontal[1]) {
    return false;
  }
  if (oneObjVertical[1] < twoObjVertical[0] || oneObjVertical[0] > twoObjVertical[1]) {
    return false;
  }
  return true;
}
...
function isInsideCircle(circle, point) {
  // if ((circle.center.x + circle.radius / 2 < point.x
  // && circle.center.x - circle.radius / 2 < point.x)
  // || (circle.center.y + circle.radius / 2 < point.y
  // && circle.center.y - circle.radius / 2 < point.y)) {
  //   return false;
  // }
...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  ...
  str += [a, b].sort((one, two) => one - two).join(', ');
  if (isEndIncluded) {
    str += ']';
  } else {
    str += ')';
 ...
 *
 */
function evaluateTicTacToePosition(list) {
  const arr = [...list];
  ...
  const resCheckRowAndColls = checkRows(arr);
  const resCheckDiagonal = checkDiagonal(arr);
  if (resCheckRowAndColls) {
    return resCheckRowAndColls;
  }
  if (resCheckDiagonal) {
    return resCheckDiagonal;
  }
  return undefined;
}
